...
try:
    ...
    raise ImportError("Требуется установить transformers: pip install transformers tokenizers") from e
...
AVAILABLE_MODELS = {
    ...
    "local_default": "gpt2"  # fallback tokenizer
}
...
def get_tokenizer_for_model(model_api_name: str):
    if model_api_name in _tokenizer_cache:
        return _tokenizer_cache[model_api_name]
    try:
        tok = AutoTokenizer.from_pretrained(model_api_name, use_fast=True)
    except Exception:
        tok = AutoTokenizer.from_pretrained("gpt2", use_fast=True)
    _tokenizer_cache[model_api_name] = tok
    return tok
...
def count_tokens_in_text(text: str, tokenizer) -> int:
    if not text:
        return 0
    MAX_CHARS_TO_TOKENIZE = 20000
    if len(text) > MAX_CHARS_TO_TOKENIZE:
...
def count_tokens_for_history(user_id, model_api_name: str) -> int:
    hist = chat_histories.get(user_id, [])
    if not hist:
        return 0
    tokenizer = get_tokenizer_for_model(model_api_name)
    total = 0
    for msg in hist:
        ...
        if isinstance(content, list):
            for item in content:
                if isinstance(item, dict) and "text" in item:
                    total += count_tokens_in_text(text_from_message_content(item["text"]), tokenizer)
                else:
                    total += count_tokens_in_text(text_from_message_content(item), tokenizer)
        else:
            total += count_tokens_in_text(text_from_message_content(content), tokenizer)
    return total

def enforce_token_limit(user_id, model_api_name: str):
    trimmed = False
    if user_id not in chat_histories or not chat_histories[user_id]:
        init_history(user_id)
        return trimmed
    total_tokens = count_tokens_for_history(user_id, model_api_name)
...
def history_usage_percent(user_id, model_api_name: str) -> int:
    t = count_tokens_for_history(user_id, model_api_name)
...
# ================= IMAGE HELPERS =================
def save_photo_bytes_to_file(bytes_data: bytes, ext="jpg") -> str:
...
def build_messages_for_api(history_list):
    ...
    for msg in history_list:
        ...
        content = msg.get("content", "")
        if isinstance(content, dict) and content.get("type") == "image":
            placeholder = f"[Image: {content.get('path')}]"
            if content.get("caption"):
                placeholder += f" Caption: {content.get('caption')}"
            out.append({"role": role, "content": placeholder})
            ...
            for item in content:
                if isinstance(item, dict) and "text" in item:
                    ...
                    parts.append(text_from_message_content(item))
            out.append({"role": role, "content": " ".join(parts)})
        else:
            out.append({"role": role, "content": text_from_message_content(content)})
    return out

# ================= KEYBOARDS =================
def main_menu_keyboard(user_id):
    s = get_settings(user_id)
    model_api_name = AVAILABLE_MODELS.get(s["model"], "gpt2")
    percent = history_usage_percent(user_id, model_api_name)
...
def callback_handler(call):
    ...
    if call.data == "main_menu":
        try:
            ...
            bot.send_message(call.message.chat.id, "Меню:", reply_markup=main_menu_keyboard(user_id))
        ...
        try:
            ...
            bot.send_message(call.message.chat.id, "Выберите модель:", reply_markup=models_keyboard(user_id))
        ...
        bot.send_message(
            ...
            f"⚡ Статус: {status}\n"
            f"🔢 Токенов (прибл.): {count_tokens_for_history(user_id, model_api_name)} / {MAX_TOKENS}",
            reply_markup=main_menu_keyboard(user_id)
        )
...
def handle_message(message):
    ...
    s = get_settings(user_id)
    model_api_name = AVAILABLE_MODELS.get(s["model"], "gpt2")
    enforce_token_limit(user_id, model_api_name)
    loading = bot.reply_to(message, "⏳ Думаю...")
    try:
        if message.photo:
            file_info = bot.get_file(message.photo[-1].file_id)
            downloaded = bot.download_file(file_info.file_path)
            local_path = save_photo_bytes_to_file(downloaded, ext="jpg")
            user_content = {"type": "image", "path": local_path, "caption": message.caption or ""}
        else:
