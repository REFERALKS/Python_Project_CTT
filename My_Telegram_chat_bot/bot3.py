...
def compress_summary(msgs):
    ...
    for msg in msgs:
        if msg["role"] == "user":
            parts.append("U: " + msg["content"][0]["text"])
        elif msg["role"] == "assistant":
            parts.append("A: " + msg["content"][0]["text"])
    return " | ".join(parts)
...
def compression_engine(user_id):
    ...
    # Stage 1: summary
    if len(history) > 12 and "summary" not in history[1]["content"][0]["text"]:
    ...
    history = chat_histories[user_id]
    if len(history) > 18 and "ultra" not in history[1]["content"][0]["text"]:
...
def send_long_message(chat_id, text, reply_markup=None):
    ...
    for i in range(0, len(text), MAX_LEN):
        bot.send_message(chat_id, text[i:i+MAX_LEN], reply_markup=reply_markup if i==0 else None)
