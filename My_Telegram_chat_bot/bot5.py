...
# Owner: highest priority always
BOT_OWNER_ID = int(os.getenv("BOT_OWNER_ID", "5178568186"))

# --- Reliability / Load ---
MAX_ACTIVE_GLOBAL = int(os.getenv("MAX_ACTIVE_GLOBAL", "1"))   # global parallelism
WORKER_COUNT = int(os.getenv("WORKER_COUNT", "1"))             # workers (1 recommended)

USER_MIN_INTERVAL_SEC = float(os.getenv("USER_MIN_INTERVAL_SEC", "2.0"))
USER_MAX_PER_MINUTE = int(os.getenv("USER_MAX_PER_MINUTE", "12"))
...
CB_RESET_TIMEOUT_SEC = float(os.getenv("CB_RESET_TIMEOUT_SEC", "20"))
...
# ---- Photo caption policy ----
AUTO_IMAGE_DESCRIPTION = (
...
EXECUTION_GUIDE = (
    ...
    "2) Если не хватает данных — сделай разумные допущения и явно их перечисли, затем задай 1–3 уточняющих вопроса.\n"
    ...
    "5) Если пользователь прислал изображение и написал подпись/текст — не описывай изображение отдельно, "
    "если он прямо не попросил. Фокусируйся на задаче из текста.\n"
    "6) Если запрос связан с причинением вреда (взлом, мошенничество, вредонос, кража данных и т.п.) — откажись "
...
# IMPORTANT: to make Telegram show code in a nice monospaced box, ask model to use ``` fences
RESPONSE_FORMAT_INSTRUCTION = (
    "Отвечай строго в формате:\n"
    "ОТВЕТ: <твой ответ>\n"
    "Не добавляй другие секции.\n"
    "Если приводишь код — обязательно оформляй его в блоках Markdown с тройными кавычками, например:\n"
    "```python\n"
    "print('hello')\n"
    "```\n"
)
...
def looks_like_code(text: str) -> bool:
    """
    Heuristic: if the answer is mostly code-like and multiline.
    We use this only if model forgot to add ``` fences.
    """
    if "```" in text:
        return False
    if "\n" not in text:
        return False
    code_markers = ("import ", "def ", "class ", "{", "};", "function ", "#include", "SELECT ", "FROM ", "->", "=>")
    hits = sum(1 for m in code_markers if m in text)
    lines = [ln for ln in text.splitlines() if ln.strip()]
    if len(lines) < 4:
        return False
    indent_lines = sum(1 for ln in lines if ln.startswith(("    ", "\t")))
    return hits >= 1 or indent_lines >= max(2, len(lines) // 3)


def parse_markdown_fences(text: str) -> List[Tuple[str, str]]:
    """
    Split text into segments: [("text", ...), ("code", ...), ...] by ``` fences.
    Supports optional language tag after opening fence.
    If fence is unclosed, treats the rest as code.
    """
    ...
    while i < n:
        ...
        # opening fence found at j
        k = j + 3
        # optional language until newline
        lang_end = text.find("\n", k)
        if lang_end == -1:
            # fence till end -> empty code
            out.append(("code", ""))
            break
        ...
        if close == -1:
            ...
            break
        out.append(("code", text[code_start:close]))
        i = close + 3

    return out


def safe_cut_no_entity(s: str, max_len: int) -> Tuple[str, str]:
    """
    Cut s into (head, tail) where len(head) <= max_len and we don't cut inside an HTML entity (&...;).
    """
    if len(s) <= max_len:
        return s, ""
    cut = max_len
    ...
    # avoid cutting inside entity: if last '&' after last ';' => cut before last '&'
    last_amp = head.rfind("&")
    last_semi = head.rfind(";")
    if last_amp != -1 and last_amp > last_semi:
        # cut before '&'
        if last_amp == 0:
            # can't make progress safely; fallback hard cut
            return head, s[cut:]
        head = head[:last_amp]
        cut = len(head)
...
def format_response_to_html(text: str) -> str:
    """
    Convert response (plain or with ``` fences) into Telegram HTML parse_mode string.
    - Normal text: escaped
    - Code blocks: <pre><code>...</code></pre> (monospace box, copy-friendly)
    """
    text = text or ""
    ...
    parts = parse_markdown_fences(text)
    if not any(t == "code" for t, _ in parts):
        return html_escape(text)

    html_parts: List[str] = []
    for kind, content in parts:
        if kind == "text":
            html_parts.append(html_escape(content))
        else:
            html_parts.append(f"<pre><code>{html_escape(content)}</code></pre>")
    return "".join(html_parts)


def split_html_for_telegram(html: str, max_len: int = 3900) -> List[str]:
    """
    Split HTML into chunks <= max_len, preserving code boxes.
    If a <pre><code> block is too large, split its content into multiple valid blocks.
    """
    segments = parse_markdown_fences(html) if False else None  # not used; html is already HTML
    # We split the already built HTML by recognizing <pre><code>... blocks.
    # Strategy: parse into tokens: text outside <pre><code> and code content inside.
    CODE_OPEN = "<pre><code>"
    CODE_CLOSE = "</code></pre>"
    ...
    i = 0
    while i < len(html):
        open_idx = html.find(CODE_OPEN, i)
        if open_idx == -1:
            # only text remains
            remaining = html[i:]
            while remaining:
                avail = max_len - len(cur)
                if avail <= 0:
                    out.append(cur)
                    cur = ""
                    avail = max_len
                head, remaining = safe_cut_no_entity(remaining, avail)
                cur += head
                if not remaining:
                    break
            break
        ...
        if open_idx > i:
            ...
            while text_part:
                ...
                if avail <= 0:
                    ...
                    cur = ""
                    avail = max_len
                head, text_part = safe_cut_no_entity(text_part, avail)
                cur += head
            i = open_idx

        # parse code block
        if html.startswith(CODE_OPEN, i):
            i += len(CODE_OPEN)
            close_idx = html.find(CODE_CLOSE, i)
            if close_idx == -1:
                # broken html, treat rest as text
                remaining = html[i:]
                while remaining:
                    avail = max_len - len(cur)
                    if avail <= 0:
            ...
            code_content = html[i:close_idx]
            i = close_idx + len(CODE_CLOSE)

            # emit code content in chunks, each wrapped
            while code_content:
                needed_wrapper = len(CODE_OPEN) + len(CODE_CLOSE)
                avail = max_len - len(cur)
                if avail < needed_wrapper + 1:
                    if cur:
                        out.append(cur)
                        cur = ""
                    avail = max_len

                cap = avail - needed_wrapper
                if cap <= 0:
                    # should not happen
                    out.append(cur)
                    cur = ""
                ...
                head, code_content = safe_cut_no_entity(code_content, cap)
                cur += CODE_OPEN + head + CODE_CLOSE
...
# =============================================================================
# TELEGRAM + OPENAI CLIENT
# =============================================================================

# We want HTML output (code boxes, monospace)
    ...
    for attempt in range(LLM_MAX_RETRIES + 1):
        try:
            ...
            if attempt >= LLM_MAX_RETRIES:
                break
            time.sleep(LLM_RETRY_BACKOFF_SEC * (2 ** attempt))
...
def get_settings(user_id: Union[int, str]) -> Dict[str, Any]:
    ...
    with STATE_LOCK:
        ...
        # migration: remove model selection
...
def build_photo_caption(user_caption: Optional[str]) -> str:
    ...
    if cap:
        return cap  # do not auto-describe if user wrote something
    return AUTO_IMAGE_DESCRIPTION
...
class Job:
    ...
    user_id: str
    chat_id: int
    origin_message_id: int  # to reply/quote in Telegram
...
def compute_priority(user_id: str, prompt_tokens_estimate: int, used_tokens_estimate: int, has_image: bool) -> int:
    try:
        if int(user_id) == BOT_OWNER_ID and BOT_OWNER_ID != 0:
...
def select_next_job_id() -> Optional[int]:
    ...
    with SCHED_LOCK:
        ...
        for u, q in user_queues.items():
            if not q or user_busy.get(u, False):
                continue
...
# =============================================================================
# TELEGRAM SEND HELPERS (HTML + code boxes + reply/quote)
# =============================================================================
...
def send_long_message_html(
    chat_id: int,
    html_text: str,
    reply_markup: Optional[Any] = None,
    reply_to_message_id: Optional[int] = None,
) -> None:
    chunks = split_html_for_telegram(html_text, max_len=3900)
...
# =============================================================================
# SNAPSHOT + TOKEN TRIM (kept from earlier bot)
# =============================================================================
...
def enforce_token_budget_strict_list(user_id: str, history: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Hard trim last message text if still too big
...
def worker_loop(worker_id: int) -> None:
    ...
    while not SHUTDOWN_EVENT.is_set():
        ...
        try:
            ...
            # Send formatted HTML to Telegram (code in boxes) and reply to original message
            html_text = format_response_to_html(response)
            send_long_message_html(
                job.chat_id,
                html_text,
                reply_markup=main_menu_keyboard(job.user_id),
                reply_to_message_id=job.origin_message_id,
            )
...
def cmd_status(message: types.Message) -> None:
    ...
    bot.reply_to(message, html_escape(text), reply_markup=main_menu_keyboard(uid(message.from_user.id)))
...
def callback_handler(call: types.CallbackQuery) -> None:
    ...
    if call.data == "main_menu":
        ...
        bot.send_message(call.message.chat.id, html_escape(msg), reply_markup=main_menu_keyboard(user_id))
        ...
        safe_edit_text(call.message.chat.id, call.message.message_id, html_escape("\n".join(lines)), reply_markup=memory_keyboard())
...
def handle_message(message: types.Message) -> None:
    ...
    if rl:
        bot.send_message(message.chat.id, html_escape(rl), reply_markup=main_menu_keyboard(user_id))
        return
    ...
    try:
        ...
        safe_delete(message.chat.id, status_msg.message_id)
        bot.send_message(message.chat.id, html_escape(f"Ошибка записи истории: {e}"), reply_markup=main_menu_keyboard(user_id))
        return

    # Priority estimate (cheap snapshot)
    try:
    ...
    pr = compute_priority(user_id=user_id, prompt_tokens_estimate=prompt_cost, used_tokens_estimate=used_cost, has_image=has_img)
    ...
    safe_edit_text(message.chat.id, status_msg.message_id, html_escape("\n".join(lines)), reply_markup=stop_keyboard(job_id))
